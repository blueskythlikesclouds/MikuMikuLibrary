...
{
    private static readonly XmlSerializer sMotionSetInfoSerializer = new(typeof(MotionSetInfo));
    ...
    protected override void Initialize()
    {
        ...
        AddImportHandler<MotionSet>(filePath =>
        {
            ...
            {
                motionSet.Load(filePath, configuration?.BoneData?.Skeletons?[0],
        ...
        AddExportHandler<MotionSet>(filePath =>
        {
            var configuration = ConfigurationList.Instance.CurrentConfiguration;
            {
                Data.Save(filePath, configuration?.BoneData?.Skeletons?[0], configuration?.MotionDatabase);
            }
        });
    ...
    protected override void Load(MotionSet data, Stream source) =>
        data.Load(source, SourceConfiguration?.BoneData?.Skeletons?[0], SourceConfiguration?.MotionDatabase);
