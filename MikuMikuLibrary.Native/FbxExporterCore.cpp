...
namespace MikuMikuLibrary::Objects::Processing::Fbx
{
    ...
    FbxNode* CreateFbxNodeFromMesh(Mesh^ mesh, Object^ object, FbxScene* lScene, List<IntPtr>^ materials, Dictionary<String^, IntPtr>^ convertedBones)
    {
        ...
        for (int i = 0; i < 1; i++)
        {
            array<Vector4>^ colors = mesh->GetColorsChannel(i);
            ...
            for each (Vector4 color in colors)
                lElementVertexColor->GetDirectArray().Add(FbxColor(color.X, color.Y, color.Z, color.W));
        }
    ...
    FbxSurfacePhong* CreateFbxSurfacePhongFromMaterial(Material^ material, TextureSet^ textureSet, String^ texturesDirectoryPath, FbxScene* lScene)
    {
        ...
        lSurfacePhong->Diffuse.Set(FbxDouble3(material->Diffuse.X, material->Diffuse.Y, material->Diffuse.Z));
        lSurfacePhong->TransparencyFactor.Set(1 - material->Diffuse.W);

        lSurfacePhong->Ambient.Set(FbxDouble3(material->Ambient.X, material->Ambient.Y, material->Ambient.Z));

        lSurfacePhong->Specular.Set(FbxDouble3(material->Specular.X, material->Specular.Y, material->Specular.Z));
        lSurfacePhong->ReflectionFactor.Set(material->Specular.W);

        lSurfacePhong->Emissive.Set(FbxDouble3(material->Emission.X, material->Emission.Y, material->Emission.Z));
