...
namespace NvTriStripDotNet
{
    ...
    internal class NvStripifier
    {
        ...
        public bool GenerateStrips( ushort[] indices, out PrimitiveGroup[] primitiveGroups,
            bool validateEnabled = false )
        {
            ...
            if ( ListsOnly )
            {
                ...
                uint numIndices = 0;
                for ( int i = 0; i < tempStrips.Count; i++ ) numIndices += ( uint ) ( tempStrips[ i ].Faces.Count * 3 );
                ...
                for ( int j = 0; j < tempStrips[ i ].Faces.Count; j++ )
            ...
            {
                stripifier.CreateStrips( tempStrips, stripIndices, StitchStrips, ref numSeparateStrips, UseRestart,
                    ( uint ) RestartValue );
                ...
                for ( int stripCtr = 0; stripCtr < numSeparateStrips; stripCtr++ )
                {
                    ...
                    primGroupArray[ stripCtr ] =
                        new PrimitiveGroup( PrimitiveType.TriangleStrip, new ushort[ stripLength ] );
                ...
                if ( tempFaces.Count != 0 )
                {
                    ...
                    primGroupArray[ faceGroupLoc ] = new PrimitiveGroup( PrimitiveType.TriangleList,
                        new ushort[ tempFaces.Count * 3 ] );
                    int indexCtr = 0;
                    for ( int i = 0; i < tempFaces.Count; i++ )
                    {
            ...
            if ( validateEnabled )
            {
                ...
                for ( int i = 0; i < numGroups; ++i )
                    switch ( primitiveGroups[ i ].Type )
                    {
                        ...
                        {
                            for ( int j = 0; j < primitiveGroups[ i ].Indices.Length; j += 3 )
                            {
                        ...
                        {
                            bool flip = false;
                            for ( int j = 2; j < primitiveGroups[ i ].Indices.Length; ++j )
                            {
        ...
        private static void Cleanup( List<StripInfo> tempStrips, List<FaceInfo> tempFaces )
        {
            ...
            for ( int i = 0; i < tempStrips.Count; i++ )
            {
                for ( int j = 0; j < tempStrips[ i ].Faces.Count; j++ ) tempStrips[ i ].Faces[ j ] = null;
            ...
            //delete faces
            for ( int i = 0; i < tempFaces.Count; i++ ) tempFaces[ i ] = null;
        }
        ...
        private static bool TestTriangle( ushort v0, ushort v1, ushort v2, List<FaceInfo>[] inBins, int numbins )
        {
            ...
            bool isLegit = false;
            int ctr = v0 % numbins;
            for ( int k = 0; k < inBins[ ctr ].Count; ++k )
                //check triangles in this bin
                if ( SameTriangle( ( ushort ) inBins[ ctr ][ k ].V0, ( ushort ) inBins[ ctr ][ k ].V1,
                    ( ushort ) inBins[ ctr ][ k ].V2,
                    v0, v1, v2 ) )
                {
                    isLegit = true;
                    break;
