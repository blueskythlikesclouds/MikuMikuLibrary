...
public class EndianBinaryReader : BinaryReader
{
    ...
    public override ulong ReadUInt64() =>
        SwapBytes ? BinaryPrimitives.ReverseEndianness(base.ReadUInt64()) : base.ReadUInt64();
    ...
    public Vector2 ReadVector2(VectorBinaryFormat format)
    {
        switch (format)
        {
            ...
            case VectorBinaryFormat.Half:
                return new Vector2((float)ReadHalf(), (float)ReadHalf());
    ...
    public Vector4 ReadVector4(VectorBinaryFormat format)
    {
        switch (format)
        {
            ...
            case VectorBinaryFormat.Half:
                return new Vector4((float)ReadHalf(), (float)ReadHalf(), (float)ReadHalf(), (float)ReadHalf());

            case VectorBinaryFormat.UInt8:
                return new Vector4(ReadByte() / 255f, ReadByte() / 255f, ReadByte() / 255f, ReadByte() / 255f);

            case VectorBinaryFormat.Int16:
                return new Vector4(ReadInt16() / 32767f, ReadInt16() / 32767f, ReadInt16() / 32767f, ReadInt16() / 32767f);
