...
namespace MikuMikuLibrary.Textures.DDS
{
    ...
    public static class DDSCodec
    {
        ...
        private static readonly RGBColour sLuminance = new RGBColour( 0.2125f / 0.7154f, 1f, 0.0721f / 0.7154f, 1f );
        private static readonly RGBColour sLuminanceInv = new RGBColour( 0.7154f / 0.2125f, 1f, 0.7154f / 0.0721f, 1f );
        ...
        /// <param name="format"></param>
        /// <returns></returns>
        public static byte[] DecompressPixelDataToRGBA( byte[] data, int width, int height,
            DDSPixelFormatFourCC format )
        {
        ...
        public static DDSPixelFormatFourCC DetermineBestCompressedFormat( Bitmap bitmap )
        {
            ...
            if ( HasTransparency( bitmap ) ) ddsFormat = DDSPixelFormatFourCC.DXT5;
        ...
        private static byte[] DecompressImageData( byte[] data, int width, int height, DDSPixelFormatFourCC format,
            bool hasHeader )
        {
            ...
            for ( int row = 0; row < texelCount; row++ )
                using ( var decompressedLine = ReadBCMipLine( compressed, height, width, bitsPerScanline, 0,
                    compressedLineSize, row, decompressBlock ) )
                {
                    int index = row * bitsPerScanline * 4;
                    decompressedLine.Position = 0;
                    int length = decompressedLine.Length > newData.Length
                        ? newData.Length
                        : ( int ) decompressedLine.Length;
        ...
        private static byte[] CompressImage( Bitmap image, DDSPixelFormatFourCC format, bool includeHeader )
        {
            ...
            {
                using ( var mipmap =
                    new UnmanagedMemoryStream( ( byte* ) imageData.Scan0, imageData.Stride * image.Height ) )
                using ( var compressed =
                    WriteMipMap( outStream, mipmap, image.Width, image.Height, pixelWriter, true ) )
                {
        ...
        private static MemoryStream ReadBCMipLine( Stream compressed, int mipHeight, int mipWidth, int bitsPerScanLine,
            long mipOffset, int compressedLineSize, int rowIndex, Func<Stream, List<byte[]>> decompressBlock )
        {
            int bitsPerPixel = 4;
            ...
            using ( var compressedLine = new MemoryStream( compressedLineSize ) )
            {
                lock ( compressed )
                {
                    ...
                    do
                    {
                        read = compressed.Read( buffer, 0, Math.Min( 4096, compressedLineSize ) );
                ...
                for ( int column = 0; column < mipWidth; column += 4 )
                {
                    ...
                    int topLeft =
                        column *
                        bitsPerPixel; // + rowIndex * 4 * bitsPerScanLine;  // Top left corner of texel IN BYTES (i.e. expanded pixels to 4 channels)
        ...
        private static MemoryStream WriteMipMap( MemoryStream outStream, Stream pixelData, int width, int height,
            Action<Stream, Stream, int, int> pixelWriter, bool isBCd )
        {
            long outStreamOrigin = outStream.Position;
            ...
            for ( int rowIndex = 0; rowIndex < texelCount; rowIndex++ )
                using ( var compressedLine = WriteMipLine( pixelData, width, height, bitsPerScanLine, isBCd, rowIndex,
                    pixelWriter ) )
                {
        ...
        private static MemoryStream WriteMipLine( Stream pixelData, int width, int height, int bitsPerScanLine,
            bool isBCd, int rowIndex, Action<Stream, Stream, int, int> pixelWriter )
        {
            var compressedLine =
                new MemoryStream(
                    bitsPerScanLine ); // Not correct compressed size but it's close enough to not have it waste tonnes of time copying.
            using ( var uncompressedLine = new MemoryStream( 4 * bitsPerScanLine ) )
            {
                lock ( pixelData )
                {
                    ...
                    do
                    {
                        read = pixelData.Read( buffer, 0, Math.Min( 4096, length ) );
                        length -= read;
                ...
                for ( int w = 0; w < width; w += isBCd ? 4 : 1 )
                {
                    ...
                    if ( isBCd && w != width - 4 && width > 4 && height > 4
                    ) // KFreon: Only do this if dimensions are big enough
                        uncompressedLine.Seek( -( bitsPerScanLine * 4 ) + 4 * 4,
                            SeekOrigin.Current ); // Not at an row end texel. Moves back up to read next texel in row.
                }
            }
        ...
        internal static byte[] Decompress8BitBlock( Stream compressed, bool isSigned )
        {
            ...
            ulong bitmask = ( ( ulong ) block[ 2 ] << 0 ) | ( ( ulong ) block[ 3 ] << 8 ) |
                            ( ( ulong ) block[ 4 ] << 16 ) | // KFreon: Read all 6 compressed bytes into single 
                            ( ( ulong ) block[ 5 ] << 24 ) | ( ( ulong ) block[ 6 ] << 32 ) |
                            ( ( ulong ) block[ 7 ] << 40 );


            // KFreon: Bitshift and mask compressed data to get 3 bit indicies, and retrieve indexed colour of pixel.
            for ( int i = 0; i < 16; i++ )
                decompressedBlock[ i ] = colours[ ( bitmask >> ( i * 3 ) ) & 0x7 ];
        ...
        private static int[] BuildRGBPalette( int colour0, int colour1, bool isDxt1 )
        {
            ...
            if ( colour0 > colour1 )
            {
                byte r1 = ( byte ) ( 2f / 3f * colour0S[ 0 ] + 1f / 3f * colour1S[ 0 ] );
                byte g1 = ( byte ) ( 2f / 3f * colour0S[ 1 ] + 1f / 3f * colour1S[ 1 ] );
                byte b1 = ( byte ) ( 2f / 3f * colour0S[ 2 ] + 1f / 3f * colour1S[ 2 ] );

                byte r2 = ( byte ) ( 1f / 3f * colour0S[ 0 ] + 2f / 3f * colour1S[ 0 ] );
                byte g2 = ( byte ) ( 1f / 3f * colour0S[ 1 ] + 2f / 3f * colour1S[ 1 ] );
                byte b2 = ( byte ) ( 1f / 3f * colour0S[ 2 ] + 2f / 3f * colour1S[ 2 ] );
            ...
            else
            {
                ...
                byte r = ( byte ) ( 1 / 2f * colour0S[ 0 ] + 1 / 2f * colour1S[ 0 ] );
                byte g = ( byte ) ( 1 / 2f * colour0S[ 1 ] + 1 / 2f * colour1S[ 1 ] );
                byte b = ( byte ) ( 1 / 2f * colour0S[ 2 ] + 1 / 2f * colour1S[ 2 ] );
        ...
        private static uint Encode565( RGBColour colour )
        {
            ...
            return ( ( uint ) ( temp.R * 31f + 0.5f ) << 11 ) | ( ( uint ) ( temp.G * 63f + 0.5f ) << 5 ) |
        ...
        private static RGBColour[] OptimiseRGB( RGBColour[] colour, int uSteps )
        {
            ...
            for ( int iteration = 0; iteration < 8; iteration++ )
            {
                ...
                for ( int i = 0; i < colour.Length; i++ )
                {
                    ...
                    float fDot = ( current.R - x.R ) * dir.R + ( current.G - x.G ) * dir.G +
                                 ( current.B - x.B ) * dir.B;
        ...
        private static byte[] CompressRGBTexel( byte[] texel, bool isDxt1, float alphaRef )
        {
            ...
            for ( int i = 0; i < texel.Length; i += 4 )
            {
                ...
                float fdot = ( current.R - step[ 0 ].R ) * dir.R + ( current.G - step[ 0 ].G ) * dir.G +
